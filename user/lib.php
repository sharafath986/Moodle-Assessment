...
function user_create_user($user, $updatepassword = true, $triggerevent = true) {
    ...
    // Create USER context for this user.
       ...
       $usercontext = context_user::instance($newuserid);
        ...
        $get_category_course = $DB->get_recordset_sql("select * from mdl_course where visible = 1 and category = 2");

        foreach($get_category_course as $course){

        $coursecontext = context_course::instance($course->id, MUST_EXIST);
        $plugin_instance = $DB->get_record("enrol", array('courseid'=>$course->id,'enrol'=>'manual'));

        $plugin = enrol_get_plugin('manual');
        $plugin->enrol_user($plugin_instance, $newuserid, $plugin_instance->roleid);	 
